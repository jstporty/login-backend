    ...
    try:
        ...
        if resp.status_code != 200:
            try:
                ...
                if 'message' in error_data:
                    ...
                    fail_reason = error_data['error']
                else:
                    fail_reason = f'HTTP {resp.status_code} - {resp.text[:100]}'
                    
                # 추가 상세 정보가 있으면 포함
                if 'errors' in error_data and error_data['errors']:
                    details = error_data['errors']
                    if isinstance(details, list):
                        fail_reason += f" (상세: {', '.join(details)})"
                    elif isinstance(details, dict):
                        fail_reason += f" (상세: {', '.join([f'{k}={v}' for k, v in details.items()])})"
                        
            except Exception as parse_error:
                fail_reason = f'HTTP {resp.status_code} - 응답 파싱 실패: {resp.text[:100]}'
        else:
            # 성공한 경우
            fail_reason = ""
        ...
        if result == "❌ FAIL":
            if expected == "SUCCESS" and actual == "실패":
                ...
                fail_reason = "[예상: 실패, 실제: 성공] 실패해야 하는데 성공함"
        ...
        fail_reason = f"❌ 요청 실패: {type(e).__name__} - {str(e)}"
        results.append({
            'No': idx,
            'TestType': '회원가입' if test_type == 'REGISTER' else '로그인',
            'Username': username,
...
ws.column_dimensions['H'].width = 50  # 실패 사유 더 넓게
