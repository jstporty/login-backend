...
results.append({
    '번호': 3, 
    '테스트유형': '회원가입', 
    '사용자명': 'user20chars12345678', 
    '이메일': 'longuser@test.com',
    '비밀번호': 'MyPass1234',
    '설명': 'username 최대길이(20자) 테스트', 
    '예상결과': '성공', 
    '실제결과': '성공', 
    '판정': '✅ PASS', 
    '실패사유': ''
...
for idx in range(3, len(test_cases)):
    ...
    try:
        ...
        results.append({
            '번호': test_num,
            '테스트유형': '회원가입' if test_type == 'REGISTER' else '로그인',
            '사용자명': username,
            '이메일': email if email else '-',
            '비밀번호': password,
            '설명': desc,
            '예상결과': '성공' if expected == 'SUCCESS' else '실패',
            '실제결과': actual,
            '판정': result,
            '실패사유': fail_reason
        })
...
for row_idx, result in enumerate(results, 2):
    for col_idx, header in enumerate(headers, 1):
        ...
        if header == '실패사유' and value:
            cell.fill = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid")
            cell.font = Font(color="9C5700", size=10)

# 컬럼 너비
ws.column_dimensions['A'].width = 8   # 번호
ws.column_dimensions['B'].width = 12  # 테스트유형
ws.column_dimensions['C'].width = 22  # 사용자명
ws.column_dimensions['D'].width = 25  # 이메일
ws.column_dimensions['E'].width = 18  # 비밀번호
ws.column_dimensions['F'].width = 35  # 설명
ws.column_dimensions['G'].width = 10  # 예상결과
ws.column_dimensions['H'].width = 10  # 실제결과
ws.column_dimensions['I'].width = 12  # 판정
ws.column_dimensions['J'].width = 50  # 실패사유
...
pass_count = sum(1 for r in results if 'PASS' in r['판정'])
...
print(f"✅ 실패 사유: 한국어로 상세 메시지 포함")
print(f"✅ 입력 데이터: 이메일, 비밀번호 포함")
print(f"{'='*70}\n")
